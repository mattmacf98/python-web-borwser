...
from URL import URL
from Utils import WIDTH, get_font, linespace
import skia
...
class Chrome:
    def __init__(self, browser):
        ...
        self.back_rect = skia.Rect.MakeLTRB(self.padding, self.urlbar_top + self.padding, self.padding + back_width, self.urlbar_bottom - self.padding)

        self.address_rect = skia.Rect.MakeLTRB(self.back_rect.top() + self.padding, self.urlbar_top + self.padding, WIDTH - self.padding, self.urlbar_bottom - self.padding)

        add_tab_width = self.font.measureText("+") + 2*self.padding
        self.new_tab_rect = skia.Rect.MakeLTRB(self.padding, self.padding, self.padding + add_tab_width, self.padding + self.font_height)
    ...
    def tab_rect(self, index):
        ...
        return skia.Rect.MakeLTRB(tabs_start + index*tab_width, self.tabbar_top, tabs_start + (index+1)*tab_width, self.tabbar_bottom)
    
    def paint(self):
        cmds = []

        cmds.append(DrawRect(skia.Rect.MakeLTRB(0, 0, WIDTH, self.bottom), 0, "white"))
